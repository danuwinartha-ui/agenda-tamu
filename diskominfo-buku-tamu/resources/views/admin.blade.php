...
<body class="bg-[#0f172a] font-sans text-slate-200">
    ...
    <div class="max-w-7xl mx-auto min-h-screen flex flex-col p-4 md:p-8">
        ...
        <div class="bg-[#1e293b] rounded-3xl shadow-xl border border-slate-700/50 p-6 mb-6 flex flex-col md:flex-row justify-between items-center gap-4 no-print">
            <div class="flex items-center gap-4">
                <img src="{{ asset('img/logo.png') }}" alt="Logo" class="max-h-12 w-auto object-contain">
                <div>
        ...
        <div class="print-header text-black">
            ...
            <p class="text-xs font-bold mt-2 border-t pt-2 italic">{{ $filterInfo }}</p>
        </div>
        ...
        <div class="bg-[#1e293b] rounded-2xl border border-slate-700/50 p-4 mb-6 flex flex-col md:flex-row items-center justify-between gap-4 shadow-lg no-print">
            <form action="{{ route('admin.dashboard') }}" method="GET" id="filterForm" class="flex flex-col md:flex-row items-center gap-3 w-full md:w-auto">
                <div class="flex items-center gap-3 bg-[#0f172a] px-4 py-2 rounded-xl border border-slate-700 focus-within:border-blue-500 transition-all">
                    ...
                    <input type="date" name="tanggal" id="inputTanggal" value="{{ request('tanggal') }}" 
            ...
            <div class="text-right">
                <div class="text-[10px] font-bold text-slate-500 uppercase tracking-widest">{{ $filterInfo }}</div>
                <div class="text-sm font-black text-white uppercase tracking-tighter">Total: <span class="text-blue-400">{{ count($guests) }}</span> Pengunjung</div>
            </div>
        </div>
                ...
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody class="divide-y divide-slate-700/50">
                        ...
                        <tr class="hover:bg-slate-800/40 transition-colors">
                            <td class="p-6">
                                <div class="text-blue-400 font-bold text-base leading-none">{{ $guest->jam_isi ?? $guest->created_at->format('H:i') }}</div>
                                <div class="text-[9px] text-slate-500 font-bold mt-1 tracking-tighter">{{ $guest->created_at->format('d/m/Y') }}</div>
                            </td>
                            <td class="p-6">
                                <div class="font-bold text-white uppercase text-base leading-tight">{{ $guest->nama }}</div>
                                <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $guest->whatsapp) }}" target="_blank" class="flex items-center gap-1.5 mt-1 group no-print">
                                ...
                                <div class="hidden print:block text-[10px] font-bold text-black">{{ $guest->whatsapp }}</div>
                                <div class="text-[11px] text-slate-400 uppercase tracking-tighter mt-1">{{ $guest->instansi }}</div>
                            </td>
                            <td class="p-6">
                                <div class="text-slate-200 italic line-clamp-2">"{{ $guest->tujuan_kunjungan }}"</div>
                                <div class="text-[11px] text-amber-400 font-bold uppercase mt-1 tracking-tighter italic">Bertemu: {{ $guest->pejabat_ditemui }}</div>
                            </td>
                            <td class="p-6">
                                <div class="flex items-center justify-center gap-4">
                                    <div class="relative cursor-pointer no-print" onclick="openPreview('{{ $guest->swafoto }}', 'Foto: {{ $guest->nama }}')">
                                        <img src="{{ $guest->swafoto }}" class="h-14 w-14 object-cover rounded-xl border-2 border-slate-600 shadow-lg hover:scale-110 transition-transform">
                                    </div>
                                    <img src="{{ $guest->swafoto }}" class="hidden print:block h-14 w-14 object-cover rounded border border-black">
                                    <div class="relative cursor-pointer no-print" onclick="openPreview('{{ $guest->tanda_tangan }}', 'TTD: {{ $guest->nama }}')">
                                        <div class="bg-white p-1 rounded-xl shadow-lg hover:scale-110 transition-transform">
                                            <img src="{{ $guest->tanda_tangan }}" class="h-10 w-20 object-contain">
                                        </div>
                                    </div>
    ...
    <script>
        @if(request('tanggal') == date('Y-m-d') || !request('tanggal'))
